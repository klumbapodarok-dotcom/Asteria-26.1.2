...
void main() {
    ...
    float fillStrength = fill * innerMask * (0.3 + core * 0.7);
    float edgeStrength = edgeBand * (0.34 + glow * 0.15);

    vec3 rgb = finalShaderCol * fillStrength + color * edgeStrength;
    float outAlpha = clamp(alpha * (fillStrength * 0.95 + edgeBand * 0.48) * mask, 0.0, 1.0);

    if (outAlpha <= 0.001) discard;
    fragColor = vec4(rgb, outAlpha) * vertexColor;
}
